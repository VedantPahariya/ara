...
class DpiMemUtil {
 public:
  ...
  // ============================================================================
  // NEW: Multicore bank-interleaved loading
  // ============================================================================
  
  /**
   * Load ELF file with bank interleaving for multicore systems
   * 
   * @param verbose Print loading progress
   * @param name Memory region name (e.g., "ram0", "ram1")
   * @param filepath Path to ELF file
   * @param num_banks Total number of interleaved banks
   * @param bank_id Which bank this is (0 to num_banks-1)
   */
  void LoadElfToNamedMemBanked(bool verbose, 
 ...
 private:
  ...
  /**
   * Write data to memory with bank interleaving
   * Only writes words that belong to the specified bank
   */
  static void WriteSegmentBanked(const MemArea &m,
                                 uint32_t offset,
